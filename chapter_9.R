...
# %%
gocheock_attendance %>%
  rep_sample_n(reps = 15, size = 30, replace = TRUE) -> gocheock_sample
...
# %%
gocheock_sample %>%
  summarise(여성합계 = sum(성별 == '여'), 여성비율 = 여성합계 / 30)
...
# %%
gocheock_attendance %>%
  rep_sample_n(reps = 15, size = 30, replace = TRUE) -> gocheock_sample2
...
# %%
gocheock_attendance %>%
  rep_sample_n(reps = 15 * 3, size = 30, replace = TRUE) %>%
  summarise(여성합계 = sum(성별 == '여'), 여성비율 = 여성합계 / 30) %>%
  summarise(여성비율_평균 = mean(여성비율))
...
# %%
gocheock_attendance %>%
  rep_sample_n(reps = 15 * 100, size = 30, replace = TRUE) %>%
  summarise(여성합계 = sum(성별 == '여'), 여성비율 = 여성합계 / 30) %>%
  summarise(여성비율_평균 = mean(여성비율))
...
crossing(
  ...
  mutate(평균 = (x + y) / 2, 편차 = 2 - 평균, 편차_제곱 = 편차^2) %>%
...
crossing(
  ...
  mutate(평균 = (x + y) / 2, 오차 = 2 - 평균, 오차_제곱 = 오차^2) %>%
...
# %%
crossing(size = c(1, 5, 10, 30, 50, 1000), reps = c(10, 100, 1000, 10000)) %>%
  rowwise() %>%
  mutate(
    sample_mean = population1 %>%
      rep_sample_n(
        size = size,
        reps = reps,
        replace = TRUE
